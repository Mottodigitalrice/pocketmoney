...
test.describe("F12 — error paths", () => {
  test.skip("approve fails with 503 → toast shows friendly network message", async ({
    page,
  }) => {
  ...
  test.skip("withdraw above balance → mapped overdraft message, not raw OVERDRAFT: code", async ({
    page,
  }) => {
  ...
  test.skip("lucky chest second open → mapped already-opened message, not raw code", async ({
    page,
  }) => {
    ...
    await expect(err).toContainText(/already opened|Monday|月曜|今週はもう/i);
  });
});
