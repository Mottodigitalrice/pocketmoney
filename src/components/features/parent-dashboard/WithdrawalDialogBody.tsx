...
/**
 * Body of the WithdrawalDialog — split out so the trigger stays in the
 * static parent-dashboard bundle while the form/dialog/select primitives
 * load only when the parent opens the dialog. F21.
 */
...
interface WithdrawalDialogBodyProps {
  childId: string;
  childName: string;
  open: boolean;
...
export function WithdrawalDialogBody({
  childId,
  childName,
  open,
  onOpenChange,
}: WithdrawalDialogBodyProps) {
